...
const DeployHelloContract = ({ walletAddress }) => {
  ...
  const handleDeploy = async () => {
    try {
      ...
      setDeployStatus("در حال دیپلوی قرارداد..."); const provider = new ethers.BrowserProvider(window.ethereum); const signer = await provider.getSigner(); const network = await provider.getNetwork(); if (Number(network.chainId) !== 16601) { setDeployStatus("لطفاً به شبکه ZenChain Testnet (Chain ID: 8408) متصل شوید."); return; }
